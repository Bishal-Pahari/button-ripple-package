...
- **Trusted Publishing (OIDC) + provenance.** Releases are published only from
  the [`publish.yml`](.github/workflows/publish.yml) GitHub Actions workflow
  using short-lived OIDC credentials — no long-lived `NPM_TOKEN` is stored. Each
  release carries a [provenance attestation](https://docs.npmjs.com/generating-provenance-statements)
  linking the published artifact to the exact source commit and workflow.
...
Consumers can verify the published artifacts came from this repository:
